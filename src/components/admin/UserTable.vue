...
<template>
  ...
  <template v-else>
    <table class="hidden w-full text-left md:table">
      <thead
        class="text-muted-foreground border-border/70 border-b text-xs tracking-wide uppercase"
      >
        <tr>
      ...
      <tbody class="divide-border divide-y">
        <tr v-for="user in users" :key="user.id" class="hover:bg-border/30 transition">
          <td class="px-5 py-3">
          ...
          <td class="text-muted-foreground px-5 py-3 text-sm">{{ user.province }}</td>
          <td class="px-5 py-3">
    ...
    <ul class="divide-border divide-y md:hidden">
      <li v-for="user in users" :key="user.id">
        <RouterLink :to="detailRoute(user)" class="hover:bg-border/30 flex items-center gap-3 p-4">
          <img
            :src="user.avatar"
            :alt="`${user.firstName} ${user.lastName}`"
            class="bg-border size-11 shrink-0 rounded-xl object-cover"
          />
          <span class="min-w-0 flex-1">
            <span class="text-foreground block truncate text-sm font-medium">
              {{ user.firstName }} {{ user.lastName }}
            </span>
            <span class="text-muted-foreground block truncate text-xs">{{ user.province }}</span>
          </span>
          <span class="flex shrink-0 flex-col items-end gap-1">
            <LevelBadge :level="user.level as LevelId" size="sm" />
            <span class="text-xs" :class="user.active ? 'text-success' : 'text-danger'">
              {{ user.active ? 'Active' : 'Deactivated' }}
            </span>
          </span>
        </RouterLink>
